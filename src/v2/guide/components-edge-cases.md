...
Если вы хотите получить доступ к корневому экземпляру созданному с помощью `new Vue` - используйте свойство `$root` в дочерних компонентах. Например: 
...
Теперь все дочерние компоненты смогут получить доступ к корневому экземпляру и использовать его в качестве глобального хранилища:
...
Затем внутри `<google-map-markers>` вы можете застать себя за созданием плохого кода, наподобие такого:
...
Возможно, вы не учли предостережения об обнаружении изменений [в массивах](list.html#Предостережения) или [в объектах](list.html#Предостережения-об-изменениях-объектов), 
или вы можете полагаться на состояния отличные от тех, что отслеживаются системой реактивности Vue.
